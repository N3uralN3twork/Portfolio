---
author: Aleksa Gordic & Matthias Quinn
title: High Performance Matrix Kernels
summary: This covers the internals of NVIDIA GPUs and some techniques used to make matrix multiplication go brrr!
cardImg: /images/NVIDIA-Logo.jpg
bannerImg: /images/NVIDIA-Logo.jpg
...
<Callout title="Reading tip" type="info">
  ...
  This is not an easy read and it's probably best if you start with a different post.
...
<Disclosure title="Matrix Multiplication">
...
![Figure 3: H100 SXM5 BF16 speed-of-light derivation](https://www.aleksagordic.com/blog/matmul/h100_sol.png)
...
![Figure 4: Power throttling reduces clock frequency and lowers the effective “speed of light”](https://www.aleksagordic.com/blog/matmul/clk.png)
...
![Figure 5: CUDA Programming Model: threads, warps, blocks, clusters, grids](https://www.aleksagordic.com/blog/matmul/cuda_model.png)
...
![Figure 7: Inside a DRAM cell: transistor + capacitor, wordline + bitline](https://www.aleksagordic.com/blog/matmul/gmem_dram_cell.png)
...
![Figure 8: GMEM model](https://www.aleksagordic.com/blog/matmul/gmem.png)
...
![Figure 9: Effect of access pattern in GMEM](https://www.aleksagordic.com/blog/matmul/gmem_example.png)
...
![Figure 10: SMEM model](https://www.aleksagordic.com/blog/matmul/smem_pt1.png)
...
![Figure 11: SMEM: good vs. bad access patterns](https://www.aleksagordic.com/blog/matmul/smem_pt2.png)
...
![Figure 12: SMEM: multicasting (served in a single cycle)](https://www.aleksagordic.com/blog/matmul/smem_pt3.png)

Figure 12: SMEM: multicasting (served in a single cycle)

Now, for the final piece of the hardware puzzle: the L1 cache.

This is an excellent [blog post](https://feldmann.nyc/blog/smem-microbenchmarks) [\[5\]](https://www.aleksagordic.com/blog/matmul#ref-5) by Axel on SMEM microbenchmarking.
...
![Figure 13: L1 cache model](https://www.aleksagordic.com/blog/matmul/l1.png)

Figure 13: L1 cache model

Let's go one level deeper and look at the tag stage and data stage in detail:

![Figure 14: Breakdown of k-way set-associative cache organization](https://www.aleksagordic.com/blog/matmul/kway.png)
...
![Figure 15: CUDA compilation flow: from CUDA C++ → PTX → SASS](https://www.aleksagordic.com/blog/matmul/ptx_sass_hierarchy.png)
...
![Figure 16: Naive CPU matmul example](https://www.aleksagordic.com/blog/matmul/cpu_kernel_example.png)
...
![Figure 17: Tile quantization](https://www.aleksagordic.com/blog/matmul/tile_quantization.png)
...
![Figure 18: Thread organization in naive matmul kernel](https://www.aleksagordic.com/blog/matmul/simple_kernel_logical.png)

Figure 18: Thread organization in naive matmul kernel

And the matmul logic itself looks like this:

![Figure 19: Naive matmul kernel](https://www.aleksagordic.com/blog/matmul/simple_kernel.png)
...
![Figure 20: Nsight Compute: Occupancy, Waves info](https://www.aleksagordic.com/blog/matmul/occupancy_waves.png)
...
![Figure 21: Wave quantization](https://www.aleksagordic.com/blog/matmul/wave_quant.png)
...
![Figure 22: PTX code corresponding to naive matmul CUDA kernel](https://www.aleksagordic.com/blog/matmul/ptx.png)
...
![Figure 23: SASS code corresponding to naive matmul CUDA kernel](https://www.aleksagordic.com/blog/matmul/sass.png)
...
![Figure 24: New logical organization of row2 and col2 variables](https://www.aleksagordic.com/blog/matmul/simple_kernel_uncoalesced_logical.png)

Figure 24: New logical organization of row2 and col2 variables

And here's what the modified kernel does now:

![Figure 25: Naive kernel with uncoalesced GMEM access](https://www.aleksagordic.com/blog/matmul/simple_kernel_uncoalesced.png)
...
![Figure 26: Roofline model](https://www.aleksagordic.com/blog/matmul/roofline.png)
...
![Figure 27: Matmul as a sum of partial outer products](https://www.aleksagordic.com/blog/matmul/outer.png)
...
![Figure 28: Dot product is equivalent to a sum of partial dot products](https://www.aleksagordic.com/blog/matmul/block_tile_equivalence.png)
...
![Figure 29: Arithmetic intensity improves when each thread computes multiple outputs and when tiles approach a square shape](https://www.aleksagordic.com/blog/matmul/arithmetic_intensity.png)
...
![Figure 30: High-level structure of the warp-tiling algorithm, also referred to as block tiling.](https://www.aleksagordic.com/blog/matmul/warp_tiling_pt1.png)
...
![Figure 32: Loading a chunk of A (GMEM) into As (SMEM)](https://www.aleksagordic.com/blog/matmul/warp_tiling_pt2_smem_load.png)
...
![Figure 33: Performing matmul between As and Bs as a series of thread-level outer products (warp-tiling + thread-tiling).](https://www.aleksagordic.com/blog/matmul/warp_tiling_pt3_compute.png)
...
![Figure 34: We keep the same high-level structure of the warp-tiling algorithm (block-tiling).](https://www.aleksagordic.com/blog/matmul/high_level_block_structure.png)
...
![Figure 36: No bank conflicts when loading rows or columns](https://www.aleksagordic.com/blog/matmul/swizzle_property.png)
...
![Figure 37: Swizzle function intuition](https://www.aleksagordic.com/blog/matmul/swizzle_function_intuition.png)
...
![Figure 38: Connecting the swizzle function to the matrix swizzle example](https://www.aleksagordic.com/blog/matmul/swizzle_apply.png)
...
![Figure 39: Why doing four 64x16 @ 16x64 wgmma calls is equivalent to doing a 64x64 @ 64x64 matmul](https://www.aleksagordic.com/blog/matmul/4_wgmma_calls.png)
...
![Figure 40: We are wasting TMA and TC cycles - we can do better](https://www.aleksagordic.com/blog/matmul/better_pipeline.png)
...
![Figure 41: More efficient TC/TMA pipeline: producer warp-group streams tiles into a circular buffer; consumer warp-group drains tiles into tensor cores.](https://www.aleksagordic.com/blog/matmul/kernel4_pipeline.png)
...
![Figure 42: Two consumer warp groups let us grow the tile from 128x128 -> 128x256 without register spills](https://www.aleksagordic.com/blog/matmul/2wgs.png)
...
![Figure 43: Persistent kernels: overlap the output store with incoming loads by launching one long-lived block per SM that processes many tiles.](https://www.aleksagordic.com/blog/matmul/overlap_stores.png)
...
![Figure 44: Naïve schedule](https://www.aleksagordic.com/blog/matmul/naive_schedule.png)

Figure 44: Naïve schedule

Can we do better? Yes—by making the schedule cache-aware:

![Figure 45: Block-wise cache-aware schedule](https://www.aleksagordic.com/blog/matmul/blockwise_schedule.png)

Figure 45: Block-wise cache-aware schedule

But can we do even better? Surprisingly, yes—by using a space-filling curve:

![Figure 46: Hilbert-curve schedule](https://www.aleksagordic.com/blog/matmul/hilbert_schedule.png)
...
![Figure 47: Using thread block clusters to reduce the number of L2/GMEM loads.](https://www.aleksagordic.com/blog/matmul/clusters.png)
...
01. NVIDIA Hopper Architecture In-Depth [https://developer.nvidia.com/blog/nvidia-hopper-architecture-in-depth/](https://developer.nvidia.com/blog/nvidia-hopper-architecture-in-depth/)
02. NVIDIA Ampere Architecture In-Depth [https://developer.nvidia.com/blog/nvidia-ampere-architecture-in-depth/](https://developer.nvidia.com/blog/nvidia-ampere-architecture-in-depth/)
03. Strangely, Matrix Multiplications on GPUs Run Faster When Given "Predictable" Data! \[short\] [https://www.thonking.ai/p/strangely-matrix-multiplications](https://www.thonking.ai/p/strangely-matrix-multiplications)
04. How CUDA Programming Works [https://www.nvidia.com/en-us/on-demand/session/gtcfall22-a41101/](https://www.nvidia.com/en-us/on-demand/session/gtcfall22-a41101/)
05. Notes About Nvidia GPU Shared Memory Banks [https://feldmann.nyc/blog/smem-microbenchmarks](https://feldmann.nyc/blog/smem-microbenchmarks)
06. CUDA Binary Utilities [https://docs.nvidia.com/cuda/cuda-binary-utilities/index.html](https://docs.nvidia.com/cuda/cuda-binary-utilities/index.html)
07. Lecture 37: Introduction to SASS & GPU Microarchitecture [https://www.youtube.com/watch?v=we3i5VuoPWk](https://www.youtube.com/watch?v=we3i5VuoPWk)
08. Dissecting the NVIDIA Volta GPU Architecture via Microbenchmarking [https://arxiv.org/abs/1804.06826](https://arxiv.org/abs/1804.06826)
09. How to Optimize a CUDA Matmul Kernel for cuBLAS-like Performance: a Worklog [https://siboehm.com/articles/22/CUDA-MMM](https://siboehm.com/articles/22/CUDA-MMM)
