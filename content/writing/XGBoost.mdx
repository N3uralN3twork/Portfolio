...
summary: The XGBoost Boosting Algorithm and an Application to the Forest Cover Type Dataset
bannerImg: /images/blog/XGBoost_logo.png
cardImg: /images/blog/XGBoost_logo.png
category: Projects
tags:
  - ML
  - Statistics
...
<figure>
<img src="/images/blog/R-Logo.png"
...
algorithm is *f*<sub>*x*</sub> , then the initial residuals are the
y-values.When this is the case, the final boosted model will be the
following equation, where $\hat{{b}}(x)$ is the weak-learning
decision tree from each iteration.

$$
\Large \hat{f}(x) = \sum_{b=1}^{B} \lambda \hat{{b}}(x)
$$
...
$$
\Large L = \sum_{i}l(y_{i}, \hat{y}) + \sum_{k}(\gamma T + \frac {1}{2} \lambda ||w||^{2})
$$
